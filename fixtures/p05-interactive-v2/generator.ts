...
function stage(def: P05InteractiveFixtureDefinition): string {
  ...
  switch (def.kind) {
    ...
    case 'star': return `${common}<svg id="star-map" class="star-map" viewBox="0 0 800 420" aria-label="relationship star map"></svg><p id="star-readout">drag and zoom</p>${controls}<button id="star-zoom" type="button">缩放星图</button></div>${close}`
    case 'orbit': return `${common}<div id="orbit-host" class="canvas-host"><canvas id="orbit-canvas"></canvas></div><p id="orbit-readout">webgl prepared</p>${controls}<button id="orbit-spin" type="button">旋转轨道</button></div>${close}`
    ...
    case 'composer': return `${common}<div id="composer-board" class="composer-board"><article id="layout-card" tabindex="0">拖拽组件</article><article class="layout-note">实时布局</article></div>${controls}<button id="layout-nudge" type="button">编排组件</button></div>${close}`
  }
}
