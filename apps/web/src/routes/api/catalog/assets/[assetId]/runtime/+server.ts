...
export const GET: RequestHandler = async (event) => {
  ...
  if (!ASSET_ID.test(params.assetId) || url.search) {
    return Response.json({ error: '无效的模板运行页请求' }, { status: 400, headers: { 'Cache-Control': 'no-store' } })
  }
  try {
    return await forwardTemplateRuntime(event, `/api/catalog/assets/${encodeURIComponent(params.assetId)}/runtime`)
  } catch {
    return Response.json({ error: '无法连接本机模板运行页服务' }, { status: 502, headers: { 'Cache-Control': 'no-store' } })
  }
}
