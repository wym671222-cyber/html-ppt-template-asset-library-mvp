...
- 下一安全动作：补齐当前 PocketBay 访问/单 Owner 安全决策、十二个生产组件 assetId 映射与既有 v1 PresentationItem 验收目标；批准的三个 retire ID 已记录在 P07 handoff。恢复只读目标核验后再次由父级确认，才可放行任何生产写入。
...
| P07 | blocked | `202c82fda92cc33b6a8d248017513b9f23859f6d` | `01a00120-bf9d-7950-a479-c15a213bed6d` | blocked | 父级独立核验：四个公开入口均 HTTP 204、无 health 200/控制会话/生产 `/data` 只读权限；缺十二项生产映射和固定 v1 PresentationItem 目标。批准的 retire IDs 已知并记录；生产迁移、备份、current 晋升、真实 retire、DB/CAS 写入均为 0 |
...
- P07 还缺生产身份合同：十二个 P05 package 使用明确的合成 ID，仓库没有它们到既有十二个静态组件的生产 assetId 映射；批准的三个合同续签 retire ID 为 `html-4d353ff9f58e974d4e8a`、`html-ef7410b3f4732cb7df31`、`html-221e1002d8e216bfefde`；旧 v1 PresentationItem 验收目标未指定。不得以标题猜测、合成 fixture 或隔离库替代。链状态已切换为 `blocked`，不继续自动动作。
