...
- 最后核实结论：P05 重开候选 `626ae6d53b4d52b63d7e618386b499e2387a4a43` 已证明 D3/Three.js/Interact.js 的真实手势；P06 全量 Vitest `43/43 files, 826/826 tests`、shell `8/8`、真实 Chrome `12/12`、生产依赖审计、扫描和构建均已完成并通过独立门禁。P07 唤醒登录后曾取得 live/ready 200 与目录 200；当前备份清单生成、隔离恢复通过，但加密归档在重新登录后仍返回 401，未进入部署写入。
- 本轮授权：用户于 2026-08-15 明确授权按原合同重开 P05；范围仅限测试视口、真实手势断言、辅助探针移除、摘要刷新和 P05 门禁复跑。
- 下一安全动作：修复或提供稳定的 PocketBay 归档下载/BFF 认证链路，取得当前状态可落地的加密 `.pba` 备份并复核回滚点；在此之前不执行部署、迁移、晋升或退役。
...
| P07 | in_progress | — | `01a00120-bf9d-7950-a479-c15a213bed6d` | pending | live/ready 与目录曾返回 200；当前清单生成与隔离恢复通过，但加密归档重试仍 401，`.pba` 回滚备份、稳定认证、固定历史演示和生产写入条件仍缺失 |
...
- P07 唤醒登录后，health live/ready 与目录只读曾返回 200；当前备份清单生成、隔离恢复通过，但当前清单加密下载在重新登录后仍返回 401，未形成稳定归档认证会话。没有读取 Cookie/storage/凭据，也没有执行生产部署请求。
- 当前生产 `.pba` 回滚备份未落地；此前旧清单的一次界面成功提示不作为当前状态回滚证据。
...
- 远端/部署：历史远端候选 `68358b95123db8735766b4443f0359a79d2a0653` 保留、不覆盖、不删除；本次新候选尚未推送。P07 未部署、未导入、未迁移、未 retire，也未写生产 DB/CAS；仅生成清单并做隔离恢复验证。
- 无关改动：无。
