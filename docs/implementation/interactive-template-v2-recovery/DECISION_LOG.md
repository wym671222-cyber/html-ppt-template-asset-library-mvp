...
| ITV2R-D13 | confirmed | P04 机械拆分为 P04A 实时预览和 P04B 目录传输；批准版本仍为 3.0，执行计划增量为 3.1 | 原 worker 在 55–60% 上下文主动返回 split_required；拆分不改变目标、范围、模型类别或验收 | P04A/P04B | 2026-08-17 |

## 审批记录

- 旧批准计划版本：2.1；旧 P03A/P03B 的事实保留，不作为 v3.0 passed 证据。
- 当前执行计划版本：3.1；用户批准版本：3.0。3.1 仅为不改变批准合同的机械拆分。
